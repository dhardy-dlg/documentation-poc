---
sidebar_position: 1
description: LLM Knowledge
---

import myImageUrl from './img/llm.webp';

# LLM

<p align="center">
  <img src={myImageUrl} alt="Exemple de bannière"  width="600"/>;
</p>
Here are some commonly used equations in the context of Large Language Models (LLMs) presented in LaTeX format:

### 1. **Cross-Entropy Loss Function**
The cross-entropy loss is a common objective function used in training language models.
$$
...
### 2. **Softmax Function**
The softmax function is used in the output layer of LLMs to convert logits into probabilities.

$$
\text{softmax}(z_i) = \frac{e^{z_i}}{\sum_{j=1}^{N} e^{z_j}}
$$

### 4. **Transformer Layer Normalization**
Layer normalization used in transformers is given by:

$$
\text{LayerNorm}(x) = \frac{x - \mu}{\sigma + \epsilon}
$$

### 5. **Feedforward Network in Transformers**
The feedforward network in transformer architecture is represented by:

$$
\text{FFN}(x) = \text{max}(0, xW_1 + b_1)W_2 + b_2
$$

### 6. **KL-Divergence for Language Model Regularization**
Kullback–Leibler divergence is used to measure the difference between two probability distributions (often used in regularizing models).

$$
D_{\text{KL}}(P \parallel Q) = \sum_{i} P(i) \log \frac{P(i)}{Q(i)}
$$
These are some of the key equations used in the training and functioning of LLMs.
